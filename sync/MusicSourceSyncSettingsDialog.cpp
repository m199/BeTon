/**
 * @file MusicSourceSyncSettingsDialog.cpp
...
#define B_TRANSLATION_CONTEXT "MusicSourceSyncSettingsDialog"
...
MusicSourceSyncSettingsDialog::MusicSourceSyncSettingsDialog(BMessenger target, int32 index,
                                       const BString &path, bool isBfs,
                                       const MusicSourceSettings *existing)
...
MusicSourceSyncSettingsDialog::~MusicSourceSyncSettingsDialog() {}

void MusicSourceSyncSettingsDialog::MessageReceived(BMessage *msg) {
  switch (msg->what) {
  case MSG_SYNC_OK:
    _SendSettings();
    break;
...
void MusicSourceSyncSettingsDialog::_SendSettings() {
  MusicSourceSettings out;
  out.path = fPath;
...
void MusicSourceSyncSettingsDialog::_BuildUI() {
  BStringView *pathView = new BStringView("path", fPath.String());

  BPopUpMenu *primaryPop = new BPopUpMenu("");
  primaryPop->AddItem(new BMenuItem(MusicSourceSettings::SourceTypeName(SOURCE_TAGS),
                                    new BMessage(MSG_PRIMARY_CHANGED)));
  if (fIsBfs) {
    primaryPop->AddItem(new BMenuItem(MusicSourceSettings::SourceTypeName(SOURCE_BFS),
                                      new BMessage(MSG_PRIMARY_CHANGED)));
  }
  primaryPop->AddItem(new BMenuItem(MusicSourceSettings::SourceTypeName(SOURCE_NONE),
                                    new BMessage(MSG_PRIMARY_CHANGED)));
  ...
  secondaryPop->AddItem(new BMenuItem(MusicSourceSettings::SourceTypeName(SOURCE_NONE),
                                      new BMessage(MSG_SECONDARY_CHANGED)));
  secondaryPop->AddItem(new BMenuItem(MusicSourceSettings::SourceTypeName(SOURCE_TAGS),
                                      new BMessage(MSG_SECONDARY_CHANGED)));
  if (fIsBfs) {
    secondaryPop->AddItem(new BMenuItem(MusicSourceSettings::SourceTypeName(SOURCE_BFS),
                                        new BMessage(MSG_SECONDARY_CHANGED)));
  ...
  conflictPop->AddItem(
      new BMenuItem(MusicSourceSettings::ConflictModeName(CONFLICT_OVERWRITE),
                    new BMessage(MSG_CONFLICT_CHANGED)));
  conflictPop->AddItem(
      new BMenuItem(MusicSourceSettings::ConflictModeName(CONFLICT_FILL_EMPTY),
                    new BMessage(MSG_CONFLICT_CHANGED)));
  conflictPop->AddItem(
      new BMenuItem(MusicSourceSettings::ConflictModeName(CONFLICT_ASK),
                    new BMessage(MSG_CONFLICT_CHANGED)));
...
void MusicSourceSyncSettingsDialog::_LoadExistingSettings() {
  if (!fExisting)
    return;

  BMenu *primaryMenu = fPrimaryMenu->Menu();
  const char *primaryName = MusicSourceSettings::SourceTypeName(fExisting->primary);
  ...
  const char *secondaryName = MusicSourceSettings::SourceTypeName(fExisting->secondary);
...
void MusicSourceSyncSettingsDialog::_UpdateControls() {
  if (!fIsBfs) {
    BMenuItem *item =
        fPrimaryMenu->Menu()->FindItem(MusicSourceSettings::SourceTypeName(SOURCE_BFS));
    if (item)
      item->SetEnabled(false);

    item = fSecondaryMenu->Menu()->FindItem(
        MusicSourceSettings::SourceTypeName(SOURCE_BFS));
    if (item)
      item->SetEnabled(false);
  }
}
