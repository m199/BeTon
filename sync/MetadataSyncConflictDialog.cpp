/**
 * @file MetadataSyncConflictDialog.cpp
...
#define B_TRANSLATION_CONTEXT "MetadataSyncConflictDialog"

MetadataSyncConflictDialog::MetadataSyncConflictDialog(BMessenger target,
...
void MetadataSyncConflictDialog::BuildLayout() {
  BString fileInfo;
...
void MetadataSyncConflictDialog::MessageReceived(BMessage *msg) {
  switch (msg->what) {
...
void MetadataSyncConflictDialog::SendChoice(uint32 what) {
  BMessage reply(what);
...
void MetadataSyncConflictDialog::UpdateTotal(int32 newTotal) {
  if (Lock()) {
    fTotal = newTotal;
    if (fFileLabel) {
      BString fileInfo;
